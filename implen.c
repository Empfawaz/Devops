...
void best_split(Example *data, int *labels, int n, int num_features, int *best_feature, double *best_threshold, double *best_gini) {
    *best_gini = 1.0;
    *best_feature = -1;
    *best_threshold = 0.0;
...
Node* build_tree(Example *data, int *labels, int n, int num_features, int max_depth, int depth) {
    ...
    if (depth == max_depth) {
        int max_label = 0;
        int max_count = 0;
        int counts[2] = {0, 0}; // Assuming binary classification (0 and 1)
        for (int i = 0; i < n; i++) {
            counts[labels[i]]++;
            if (counts[labels[i]] > max_count) {
                max_count = counts[labels[i]];
                max_label = labels[i];
            }
        }
        node->is_leaf = 1;
        node->class_label = max_label;
        return node;
    }
    ...
    double best_gini;
    best_split(data, labels, n, num_features, &best_feature, &best_threshold, &best_gini);
    ...
    node->left = build_tree(left_data, left_labels, left_count, num_features, max_depth, depth + 1);
    node->right = build_tree(right_data, right_labels, right_count, num_features, max_depth, depth + 1);
...
int main() {
    ...
    Node *root = build_tree(data, labels, num_examples, num_features, max_depth, 0);
